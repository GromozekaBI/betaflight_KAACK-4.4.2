...
static void applyRpmLimiter(mixerRuntime_t *mixer)
{
    ...
    DEBUG_SET(DEBUG_BIG_BLACK, 0, lrintf(unsmoothedAverageRpm)); // c учетом фильтрации
    DEBUG_SET(DEBUG_BIG_BLACK, 1, lrintf(mixer->rpmLimiterThrottleScale * 100.0f)); // текущий расчетный предел дросельной заслонки
    DEBUG_SET(DEBUG_BIG_BLACK, 2, lrintf(throttle * 100.0f)); //текущее значение газа
    DEBUG_SET(DEBUG_BIG_BLACK, 3, lrintf(mixer->rpmLimiterRpmLimit * 1.0f)); //Значение ограничителя в данный момент
    DEBUG_SET(DEBUG_BIG_BLACK, 4, lrintf(scale_dynamic_rpm_limit * 1.0f)); //Расчетное значение динамического ограничителя
