...
static void applyRpmLimiter(mixerRuntime_t *mixer)
{
    ...
    //DEBUG_SET(DEBUG_BIG_BLACK, 2, lrintf(mixer->rpmLimiterThrottleScale * 100.0f)); // текущий расчетный предел дросельной заслонки
    //DEBUG_SET(DEBUG_BIG_BLACK, 3, lrintf(throttle * 100.0f)); //текущее значение газа
    ...
    //DEBUG_SET(DEBUG_BIG_BLACK, 0, lrintf(attitude.raw[FD_ROLL]));
    //DEBUG_SET(DEBUG_BIG_BLACK, 0, lrintf(attitude.raw[FD_PITCH]));
